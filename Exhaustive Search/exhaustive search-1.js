...
function solution(sizes) {
    // 명함 크기의 가로, 세로 길이를 [작은수, 큰수] 형태로 바꿔준다.
    const reSizes = sizes.map(([a, b]) => a > b ? [b, a] : [a, b]);

    // 각 가로와 세로만 모아둔 새로운 배열을 만든다.
    const width = reSizes.map(el => el[0]);
    const height = reSizes.map(el => el[1]);
